...
from iflow_agent2.runtime.env import Action, Environment, Observation
...
class Runner:
    ...
    async def run(self, agent: BaseAgent, env: Environment) -> RunResult:
        ...
        for t in range(max_steps):
            ...
            try:
                step_kwargs = {"observation": obs, "history": history}
                step_parameters = inspect.signature(agent.step).parameters
                if "current_step" in step_parameters:
                    step_kwargs["current_step"] = t + 1
                if "max_steps" in step_parameters:
                    step_kwargs["max_steps"] = max_steps
                if self.step_timeout:
                    step_result = await asyncio.wait_for(
                        agent.step(**step_kwargs),
                        timeout=self.step_timeout,
                    )
                else:
                    step_result = await agent.step(**step_kwargs)
