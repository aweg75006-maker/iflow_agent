"""MainAgent coordination and task delegation."""
...
from iflow_agent2.base.engine.async_llm import ModelPricing
from iflow_agent2.base.engine.logs import LogLevel, logger
from iflow_agent2.master.common.utils import indent_text, parse_json_response
from iflow_agent2.runtime.env import BasicInfo
...
class MainAgent(BaseAgent):
    """Coordinate a task by delegating work to SubAgents."""
    
    name: str = Field(default="MainAgent")
    description: str = Field(default="Multi-agent task coordinator")
    ...
    def _format_subtask_history(self) -> str:
        ...
        summary_lines = ["---", f"Summary: {done_count}/{total_subtasks} subtasks done"]
    ...
    async def step(self, observation, history, **kwargs) -> tuple:
        """Execute one coordination decision."""
        self.attempt += 1
        ...
        # Get LLM decision
        logger.info("[MainAgent] Calling LLM...")
...
Return JSON: {{"action": "...", "reasoning": "...", "params": {{...}}}}"""
            ...
            if subtask_results:
                ...
                for sub_result in subtask_results:
                    ...
                    sub_entry = {
                        ...
                        "input_tokens": sub_result.get('input_tokens', 0),
                        "output_tokens": sub_result.get('output_tokens', 0),
                        "tools_assigned": sub_result.get('tools_assigned'),
                        "trace": sub_result.get('trace', []),
                        "result": entry_result,
                ...
                self.task_entries.append({
                    ...
                    "tools_assigned": result.get('tools_assigned'),
    ...
    async def run(self, request: Optional[str] = None) -> str:
        return "MainAgent execution is managed by Runner"
