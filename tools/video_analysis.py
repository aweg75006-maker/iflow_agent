"""Video analysis tool for local media files.
...
- If audio exists, extracts it via ffmpeg and transcribes it with the configured ASR model.
...
from iflow_agent2.base.engine.async_llm import LLMsConfig, llm_retry
from iflow_agent2.base.engine.logs import logger
from iflow_agent2.tools.audio_analysis import (
    MAX_SEGMENT_SECONDS,
    build_audio_content,
    get_wav_duration,
    split_wav_segments,
...
class VideoAnalysisAction(BaseAction):
    ...
    Processing pipeline:
    1. Send small videos directly to MiMo vision, with FFmpeg frames as fallback.
    2. Detect whether the video has an audio track (via ffprobe).
    3. If audio exists, extract it via ffmpeg and transcribe with MiMo ASR.
    ...
    description: str = (
        "Analyze video content with direct MiMo video input or extracted key frames, "
        "plus the audio track when present. "
    ...
    parameters: Dict[str, Any] = {
        ...
        "properties": {
            ...
            "direct_video": {
                "type": "boolean",
                "description": (
                    "Send a small complete video directly to MiMo before falling back "
                    "to FFmpeg frame extraction (default: true)."
                ),
            },
        },
    ...
    AUDIO_MODEL: str = "mimo-asr"
    # Model for combining visual + audio results into a final answer
    SUMMARY_MODEL: str = "mimo-pro"
    DIRECT_VIDEO_MAX_BYTES: int = 20 * 1024 * 1024
    ...
    def _extract_frames(
        ...
        """Extract JPEG frames with FFmpeg, falling back to OpenCV."""
        ffmpeg = self._get_ffmpeg()
        if ffmpeg:
            duration = self._get_video_duration(video_path) or 300
            bounded_start = max(0.0, start_time)
            bounded_end = min(end_time or duration, duration)
            segment_duration = max(0.1, bounded_end - bounded_start)
            interval = max(1.0, segment_duration / max_frames)
            try:
                with tempfile.TemporaryDirectory(prefix="iflow-video-frames-") as tmp_dir:
                    output_pattern = str(Path(tmp_dir) / "frame_%03d.jpg")
                    result = subprocess.run(
                        [
                            ffmpeg,
                            "-v", "error",
                            "-ss", str(bounded_start),
                    ...
                    if result.returncode == 0:
                        ...
                        if frame_paths:
                            logger.info(
                                f"[VideoAnalysis] FFmpeg extracted {len(frame_paths)} frames"
                            )
                            return [
                    ...
                    logger.warning(
                        "[VideoAnalysis] FFmpeg frame extraction failed; "
                        f"falling back to OpenCV: {result.stderr[:300]}"
                    )
            except (OSError, subprocess.TimeoutExpired) as exc:
                logger.warning(
                    f"[VideoAnalysis] FFmpeg frame extraction unavailable ({exc}); "
                    "falling back to OpenCV"
                )

        return self._extract_frames_opencv(
            video_path, max_frames, start_time, end_time
        )
    ...
    def _extract_frames_opencv(
        ...
        """OpenCV fallback for environments without a working FFmpeg binary."""
                ...
                completion = await llm_retry(
                    ...
                    messages=[{
                        ...
                        "content": build_audio_content(
                            ...
                            "Please transcribe it and describe notable sounds. "
                            f"Context: {query}",
                        ),
                    }],
    ...
    async def _analyze_direct_video(
        ...
        """Send a small local video directly to MiMo's video_url input."""
        if model.lower() != "mimo-v2.5":
            return None
        if os.path.getsize(video_path) > self.DIRECT_VIDEO_MAX_BYTES:
            logger.info(
                "[VideoAnalysis] Video exceeds direct-input limit; using FFmpeg frames"
            )
            return None
        ...
        mime_type = mime_by_suffix.get(suffix, "video/mp4")
        ...
        try:
            ...
            result = completion.choices[0].message.content
            if result:
                logger.info("[VideoAnalysis] Direct MiMo video analysis completed")
                return result.strip()
        except Exception as exc:
            logger.warning(
                f"[VideoAnalysis] Direct video input failed ({exc}); "
                "falling back to FFmpeg frames"
            )
        return None
    ...
    async def __call__(self, **kwargs) -> Any:
        ...
        direct_video = kwargs.get("direct_video", True)
        if end_time is not None:
        ...
        # ===== Step 1: Visual analysis (direct video, then frame fallback) =====
        visual_result: Optional[str] = None
        direct_video_analyzed = False
        if api_key and direct_video and start_time == 0 and end_time is None:
            visual_result = await self._analyze_direct_video(
                video_path, str(query), api_key, base_url, model
            )
            direct_video_analyzed = visual_result is not None

        frames: List[str] = []
        if api_key and visual_result is None:
            logger.info(
        ...
        if not api_key:
            logger.warning(
                "[VideoAnalysis] Vision model is not configured; skipping frames "
                "and continuing with the audio track."
            )

        if visual_result is None and frames:
            logger.info(
        ...
        if visual_result and audio_result:
            ...
            return {
                ...
                "metrics": {
                    ...
                    "direct_video_analyzed": direct_video_analyzed,
            ...
            return {
                ...
                "output": None,
                "error": (
                    "No video result was produced. Configure the `vision` model "
                    "for frame understanding, or provide a video with an audio "
                    "track that MiMo ASR can transcribe."
                ),
                "metrics": {},
            }
