...
def build_audio_content(model: str, audio_b64: str, fmt: str, prompt: str) -> list:
    """Build OpenAI-compatible audio content with MiMo ASR's audio-only rule."""
    audio_part = {
        "type": "input_audio",
        "input_audio": {"data": audio_b64, "format": fmt},
    }
    if model.lower() == "mimo-v2.5-asr":
        return [audio_part]
    return [{"type": "text", "text": prompt}, audio_part]
...
class ParseAudioAction(BaseAction):
    """Transcribe and analyze audio with an OpenAI-compatible model.

    Uses an OpenAI-compatible input_audio model.
    Long audio files (>30s) are automatically split into segments and summarized.
    """

    name: str = "ParseAudioAction"
    description: str = (
        "Transcribe and analyze an audio clip using the configured audio model. "
    ...
    async def _transcribe_segment(
        ...
        completion = await llm_retry(
            ...
            messages=[{
                "role": "user",
                "content": build_audio_content(model, audio_b64, fmt, query),
            }],
    ...
    async def __call__(self, **kwargs) -> Any:
        ...
        if not api_key:
            return {
                "success": False, "output": None,
                "error": f"No API key found for audio model {self.AUDIO_MODEL!r}",
                "metrics": {},
            }
