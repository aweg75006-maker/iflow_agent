"""Asynchronous model configuration and invocation helpers."""
import asyncio
import os
import re
...
from iflow_agent2.base.engine.cost_monitor import record_cost
...
class LLMsConfig:
    ...
    @classmethod
    def default(cls):
        """Get the package model configuration, using the original YAML format."""
        if cls._default_config is None:
            config_data: Optional[Dict[str, Any]] = None

            explicit_path = os.getenv("IFLOW_AGENT_MODEL_CONFIG")
            package_path = Path(__file__).resolve().parents[2] / "config" / "model_config.yaml"
            config_paths = [Path(explicit_path).expanduser()] if explicit_path else []
            config_paths.append(package_path)
            ...
            if config_file is not None:
                ...
                config_data = cls._resolve_env_values(config_data)
            else:
    ...
    @classmethod
    def load(cls, path: str | Path) -> "LLMsConfig":
        """Load one YAML file without changing the process-wide default."""
        config_path = Path(path).expanduser()
        with open(config_path, "r", encoding="utf-8") as f:
            config_data = yaml.safe_load(f) or {}
        config_data = cls._resolve_env_values(config_data)
        return cls(config_data.get("models", config_data) or {})

    @classmethod
    def reset_default(cls) -> None:
        """Clear the cached default, mainly for tests and config reloads."""
        cls._default_config = None

    @classmethod
    def _resolve_env_values(cls, value: Any) -> Any:
        """Resolve ${NAME} and ${NAME:-default} placeholders recursively."""
        if isinstance(value, dict):
            return {key: cls._resolve_env_values(item) for key, item in value.items()}
    ...
    def _load_config_from_env(cls) -> Optional[Dict[str, Any]]:
        ...
        inline_config = os.getenv("IFLOW_MODEL_CONFIG_JSON")
        if inline_config:
            try:
                ...
                logger.log_to_file(
                    LogLevel.WARNING,
                    "Failed to parse IFLOW_MODEL_CONFIG_JSON; falling back to explicit env vars.",
                )
            else:
                if isinstance(data, dict):
                    return data.get("models", data) or {}

        api_key = os.getenv("IFLOW_OPENAI_API_KEY") or os.getenv("OPENAI_API_KEY")
        if not api_key:
            return None

        base_url = (
            os.getenv("IFLOW_OPENAI_BASE_URL")
        ...
        temperature = _get_float("IFLOW_OPENAI_TEMPERATURE", 1)
        top_p = _get_float("IFLOW_OPENAI_TOP_P", 1)

        models_env = os.getenv("IFLOW_OPENAI_MODELS", "o3")
        ...
        for model_name in models:
            ...
            env_key_name = f"IFLOW_{normalized}_API_KEY"
            env_base_name = f"IFLOW_{normalized}_BASE_URL"
    ...
    def get(self, llm_name: str) -> LLMConfig:
        ...
        if not llm_config["key"]:
            raise ValueError(
                f"API key for {llm_name!r} is empty. Set the environment variable "
                "referenced by iflow_agent2/config/model_config.yaml."
            )
